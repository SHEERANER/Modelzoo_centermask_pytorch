...
def mask_rcnn_loss(pred_mask_logits, instances, maskiou_on, weight = None):
    """
    ...
    # lzy 12.20 when use func clamp,it need the cpu tensor
           # mask_ratios.append(
           #    (cropped_mask / instances_per_image.gt_masks.area())
           #     .to(device=pred_mask_logits.device).clamp(min=0., max=1.)
           # )
            unNamedComputeCputensor = (cropped_mask/ instances_per_image.gt_masks.area()).to('cpu').clamp(min=0.,max=1.)
            mask_ratios.append(torch.tensor(unNamedComputeCputensor,dtype=torch.float32).to(device=pred_mask_logits.device))
     # end transform
    ...
    if weight != None:
        mask_loss = mask_loss.mean(dim=(1,2)) * weight.float()

    mask_loss = mask_loss.sum()/weight.sum()
    ...
    if maskiou_on:
        ...
        data_type = mask_ratios[0].dtype
        value_eps = 1e-10 * torch.ones(gt_masks.shape[0], device=gt_masks.device).to(data_type)
        ...
        data_type_2 = mask_union_area[0].dtype
        value_1 = torch.ones(pred_masks.shape[0], device=gt_masks.device).to(data_type_2)
...
def mask_rcnn_inference(pred_mask_logits, pred_instances):
    ...
    if cls_agnostic_mask:
        ...
        # import pdb;pdb.set_trace()
        indices = torch.arange(num_masks, device=class_pred.device).long()
