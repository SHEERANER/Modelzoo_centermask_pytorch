...
##edit by zsc: add code to provide the dynastic shape
def select_foreground_proposals_fix_shape( proposals, bg_label):
    """
    Given a list of N Instances (for N images), each containing a `gt_classes` field,
    return a list of Instances that contain instances with `gt_classes != -1 &&
    gt_classes != bg_label` .
    In order to solve the task of dynasmic shape, we set the max instances per image 
    is 128 and fix the shape problem.
    ...
    assert proposals[0].has("gt_classes")
    fg_proposals = []
    fg_selection_masks = torch.empty(0).bool()
    for proposals_per_image in proposals:
        gt_classes = proposals_per_image.gt_classes
        fg_selection_masks = fg_selection_masks.to(gt_classes.device)
        fg_selection_mask = (gt_classes != -1) & (gt_classes != bg_label)
        fg_idxs = fg_selection_mask.nonzero().squeeze(1)
        fix_num = 128 #128 is the max instances
        pos_idxs = torch.zeros(fix_num, device = fg_selection_mask.device).long()
        weight_mask = pos_idxs.bool().scatter(0,fg_idxs,(fg_idxs+1).bool())
        pos_idxs = pos_idxs.scatter(0,fg_idxs,fg_idxs)

        fg_proposals.append(proposals_per_image[pos_idxs])
        fg_selection_masks = torch.cat((fg_selection_masks,weight_mask),0)
    return fg_proposals, fg_selection_masks
###end
...
class ROIHeads(nn.Module):
    ...
    def _sample_proposals(self, matched_idxs, matched_labels, gt_classes):
        ...
        sampled_idxs = torch.cat([sampled_fg_idxs.cpu(), sampled_bg_idxs.cpu()], dim=0).to(device).long()
    ...
    def label_and_sample_proposals(self, proposals, targets):
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            # martix = torch.load("/home/zsclzy/centermask_npu2/martix.pt")
            # matched_idxs_new, matched_labels_new = self.proposal_matcher(martix)
...
class CenterROIHeads(ROIHeads):
    ...
    def forward(
        ...
        del images
        if self.training:
            # print("before label_and_sample_proposals", datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f'))
            proposals = self.label_and_sample_proposals(proposals, targets)
            # print("after label_and_sample_proposals", datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f'))
        del targets

        if self.training:
            if self.maskiou_on:
                # print("before forward_mask", datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f'))
                #import pdb;pdb.set_trace()
                losses, mask_features, selected_mask, labels, maskiou_targets, weight = self._forward_mask(features, proposals)
                losses.update(self._forward_maskiou(mask_features, proposals, selected_mask, labels, maskiou_targets, weight))
            else:
    ...
    def _forward_mask(
        ...
        if self.training:
            ...
            proposals, weight = select_foreground_proposals_fix_shape(instances, self.num_classes)
            # proposal_boxes = [x.proposal_boxes for x in proposals]
            mask_features = self.mask_pooler(features, proposals, self.training)
            mask_logits = self.mask_head(mask_features)
            if self.maskiou_on:
                loss, selected_mask, labels, maskiou_targets = mask_rcnn_loss(mask_logits, proposals, self.maskiou_on, weight)
                return {"loss_mask": loss}, mask_features, selected_mask, labels, maskiou_targets, weight
            else:
    ...
    def _forward_maskiou(self, mask_features, instances, selected_mask=None, labels=None, maskiou_targets=None, weight = None):
        """
        ...
        if self.training:
            ...
            return {"loss_maskiou": mask_iou_loss(labels, pred_maskiou, maskiou_targets, self.maskiou_weight, weight)}
