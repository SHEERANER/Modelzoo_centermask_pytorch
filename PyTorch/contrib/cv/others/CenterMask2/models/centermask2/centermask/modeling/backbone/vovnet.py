...
class _OSA_module(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.identity:
            ...
            # x.retain_grad()
            # xt.retain_grad()
            # xt.backward(torch.ones(xt.size()).npu(),retain_graph=True)
            xt = (xt+ identity_feat)
            # xt.backward(torch.ones(xt.size()).npu(),retain_graph=True)
...
class VoVNet(Backbone):
    ...
    def forward(self, x):
        ...
        #  outputs["stage5"].backward(torch.ones(outputs["stage5"].size()).npu(),retain_graph=True)
        # outputs["stage4"].backward(torch.ones(outputs["stage4"].size()).npu(),retain_graph=True)
        # outputs["stage3"].backward(torch.ones(outputs["stage3"].size()).npu(),retain_graph=True)
        # outputs["stem"].backward(torch.ones(outputs["stem"].size()).npu(),retain_graph=True)
        return outputs
